...
def tractFindVisits(rerun, tract, filter='HSC-I', patch=None,
    ...
    if patch is not '':
        ...
        print "\n# Input visits for Tract=%d Filter=%s Patch=%s\n" % (tract, filter, patch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataDir', '-d', dest='dataDir', help="Data location",
            default='/lustre/Subaru/SSP/rerun/')
    parser.add_argument('--filter', '-f', dest='filter', help="Filter name",
            default='HSC-I')
    parser.add_argument('--patch', '-p', dest='patch', help="Patch ID",
            default='')
